...
uint16_t Geet::uint16() {
    return geet_rand16();
}

uint32_t Geet::uint32() {
    return geet_rand32();
}

WaveType Geet::type() {
    // WaveType ha 3 valori validi: WAVE_UNKNOWN, HAARP, AIO
    return static_cast<WaveType>(geet_rand8() % 3);
}
